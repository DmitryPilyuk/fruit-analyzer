...
class Model:
    ...
    def get_prediction(self, image_path):
        numpy_probabilities_dict = infer(self.__model, image_path, self.__feature_group)['probabilities']

        output_dict = {str(key): float(value) for key, value in numpy_probabilities_dict.items()}
        return output_dict
